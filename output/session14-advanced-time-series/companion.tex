...
\section{ARMA Models: Combining Memory and Shocks}
\secsub{Combine Autoregressive past memory with Moving Average shock corrections.}

The Autoregressive Moving Average model ARMA($p, q$) combines autoregressive terms (past values $y_{t-i}$) with moving average terms (past residual shocks $\epsilon_{t-j}$):
\begin{equation}
y_t = c + \phi_1 y_{t-1} + \dots + \phi_p y_{t-p} + \epsilon_t + \theta_1 \epsilon_{t-1} + \dots + \theta_q \epsilon_{t-q}
\end{equation}

\begin{intuition}
Your morning mood depends on yesterday's mood (Autoregressive memory) plus how much of yesterday's unexpected good/bad surprise still lingers today (Moving Average shock).
\end{intuition}
...
\end{equation}
\item Un-difference to obtain level forecast $\hat{y}_{101}$:
\begin{equation}
...
\begin{watchout}
VAR models require all target series to be stationary. If series have unit roots, difference them first or fit a Vector Error Correction Model (VECM).
\end{watchout}

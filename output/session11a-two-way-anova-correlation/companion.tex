...
\section{Simple Linear Regression \& Coefficient of Determination ($R^2$)}
\secsub{Predict a target variable and measure explained variation via $R^2 = SSR / SST$.}

Simple linear regression models the target variable $Y$ as a linear function of predictor $X$ plus random noise: $Y = a + bX + \epsilon$. The Coefficient of Determination ($R^2 = r^2$) measures the proportion of total variation ($SST = SSR + SSE$) explained by the regression line ($SSR$).

\begin{intuition}
Think of simple regression as drawing a baseline path through scattered data points so that total missed vertical distances ($SSE$) are minimized, while $R^2$ tells you what percentage of scatter is explained by your path.
\end{intuition}

\subsection{Least Squares \& $R^2$ Formulas}
Slope $b$, intercept $a$, and Coefficient of Determination $R^2$:
\begin{align}
b &= \frac{\sum (X - \bar{X})(Y - \bar{Y})}{\sum (X - \bar{X})^2}, \quad a = \bar{Y} - b\bar{X} \\
R^2 &= \frac{SSR}{SST} = 1 - \frac{SSE}{SST} = (r_{X,Y})^2
\end{align}
...
Suppose $SST = 42.00$ and $SSR = 35.29$.
\begin{steps}
...
\end{equation}
\item Calculate Coefficient of Determination $R^2$:
\begin{equation}
R^2 = \frac{SSR}{SST} = \frac{35.29}{42.00} = 0.8402 \quad (84.02\% \text{ of revenue variation is explained by ad spend})
\end{equation}
...
\item Least-squares regression minimizes squared vertical errors; $R^2 = SSR / SST$ measures fit quality.
\end{itemize}
\end{keytake}

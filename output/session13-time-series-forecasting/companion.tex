...
\section{Moving Averages (Simple \& Weighted)}
\secsub{Smooth out short-term fluctuations by averaging nearby historical observations.}

Moving averages assume that future observations will resemble recent historical levels. Simple Moving Averages (SMA) weigh all $k$ recent periods equally, whereas Weighted Moving Averages (WMA) assign larger weights to more recent observations.

\begin{intuition}
A simple moving average is like taking the average temperature over the last 3 days to guess tomorrow's weather. A weighted moving average gives yesterday 50\% weight, two days ago 30\%, and three days ago 20\%.
\end{intuition}

\subsection{Mathematical Formulas}
\begin{align}
\text{SMA}_t &= \frac{y_{t-1} + y_{t-2} + \dots + y_{t-k}}{k} \\
\text{WMA}_t &= \frac{w_1 y_{t-1} + w_2 y_{t-2} + \dots + w_k y_{t-k}}{\sum w_i}
\end{align}
...
\begin{intuition}
If sales grow by 10 units every month, simple smoothing will always underestimate next month's sales. Holt's method tracks both the current height and the speed of growth.
\end{intuition}

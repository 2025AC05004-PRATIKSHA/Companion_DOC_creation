...
%  using the callouts (incl. the new waitwhat) + a fully worked example, so the template renders
...
%  section will prove ("Three moves clean any system; the staircase shows every
%  answer."), never as an agenda ("In this section we will...").
% -----------------------------------------------------------------------------
\newcommand{\secsub}[1]{%
  \noindent{\sffamily\bfseries\itshape\color{navy!85}\large #1}\par\medskip}

% -----------------------------------------------------------------------------
%  \flipanswer{...} — an upside-down answer for self-test questions, so the
%  reader must try before reading. Use inside the closing self-test:
%  "Q3. Can a system have exactly two solutions?  \flipanswer{No — 0, 1 or
%  infinity. Two crossing points would force a whole line of them.}"
% -----------------------------------------------------------------------------
\newcommand{\flipanswer}[1]{\rotatebox[origin=c]{180}{\parbox{0.92\linewidth}{\small #1}}}

% =============================================================================
%  PILL-TAB CALLOUTS  —  the SIX taxonomy boxes
...
% --- Wait, what? (teal, glyph ❠) — the honest surprise ------------------------
%  One per concept AT MOST, and only where a consequence is genuinely startling
%  ("turn every dial, land on the same spot"). Never forced; an empty slot is
%  better than a fake surprise. Word-only, like intuition/everyday.
...
mistake; the \textcolor{waitFrame}{\textbf{teal}} ``wait --- really?'' box holds
a consequence so surprising it sounds wrong (it isn't); and the
\textcolor{keyFrame}{\textbf{purple}} ``key takeaway'' box is the one sentence to
remember. Read it alongside the slides, in order.

\medskip

% ============================================================================
%  AGENT:FILL — OPENING SECTION(S)
%  OPEN BY DOING, not with an agenda: the first section lets the reader DO the
%  lecture's central thing in miniature, by feel, before any definitions
%  ("Solve one right now"), then names what they just did. If a short map of
%  the reader helps, it goes AFTER that first taste, never first. Then one
%  \section per slide-concept, each opening with \secsub{the whole idea in one
%  line}. Use \vocab{...} on each term's first use, wrap display math in
%  align/split, and add a \housefig per matplotlib plot.
%  Example skeleton (uncomment + fill, or duplicate the specimen below):
%
%  \section{ {{HEADLINE_TITLE — a claim, not a label}} }
%  \secsub{ {{THE_WHOLE_IDEA_IN_ONE_OR_TWO_PLAIN_SENTENCES}} }
...
%  \begin{waitwhat}  {{ONLY if a consequence is genuinely startling}} \end{waitwhat}
...
%  Uses the callouts + one fully worked example. The figure here is an
...
\secsub{One yes/no trial, one number $p$ for the chance of yes --- that is the
whole distribution.}
